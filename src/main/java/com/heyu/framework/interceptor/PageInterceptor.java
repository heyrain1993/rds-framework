...
/**
 * 分页原理：mybatis在执行SQL语句之前会产生包含SQL语句的statement对象；对应的SQL语句在statement之前产生的，
 * 因此在成statement之前对SQL下手；mybatis的statement是通过RoutingStatementHandler的prepare方法产生的，
 * 因此拦截器的一个分页思路就是拦截prepare方法；在拦截器方法中把Sql语句改成对应的分页查询Sql语句，
...
public class PageInterceptor implements Interceptor {
    ...
    /**
     * 拦截方法@Intercepts表示当前对象是一个interceptor;mybatis拦截器只能拦截：Executor、StatementHandler、
     * ParameterHandler和ResultSetHandler类型的接口
     * 注解@Signature表名要拦截的接口类型、方法方法、对应的参数类型
     * @param invocation
     * @return
     * @throws Throwable
     */
    ...
    public Object intercept( Invocation invocation) throws Throwable {
        ...
        * 3.Mybatis在进行Sql语句处理的时候都是建立的RoutingStatementHandler，而在RoutingStatementHandler里面拥有一个
        *   StatementHandler类型的delegate属性，
        * 4.RoutingStatementHandler会依据Statement的不同建立对应的BaseStatementHandler，即SimpleStatementHandler、
        *   PreparedStatementHandler或CallableStatementHandler，
        * 5.在RoutingStatementHandler里面所有StatementHandler接口方法的实现都是调用的delegate对应的方法。
        * 6.我们在PageInterceptor类上已经用@Signature标记了该Interceptor只拦截StatementHandler接口的prepare方法，
        ...
        *   是通过Interceptor的plugin方法进行包裹的，所以我们这里拦截到的目标对象肯定是RoutingStatementHandler对象。
        ...
        Object paramters = boundSql.getParameterObject();

        Page<Object> page = (Page) ReflectUtils.getValueByFieldName(paramters,"page");
        //这里我们简单的通过传入的是Page对象就认定它是需要进行分页操作的。
        if(page != null && page.getUseFlag() == true && page.getPageNo() > -1 && page.getPageSize() > 0){
    ...
    public void setProperties(Properties properties) {
        this.setDataBaseType(properties.getProperty("dataBaseType"));
    }
